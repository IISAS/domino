...
class WorkspaceRepository(object):
    ...
    def find_by_id_and_user(self, id: int, user_id: int) -> Workspace:
        with session_scope() as session:
            """
            SELECT workspace.id, workspace.name, user_workspace_associative.permission
            FROM workspace
    ...
    def find_by_id_and_user_id(self, id: int, user_id: int) -> Tuple[Workspace, UserWorkspaceAssociative]:
        with session_scope() as session:
            query = session.query(Workspace.id.label('workspace_id'), Workspace.name, UserWorkspaceAssociative.permission.label('permission'), UserWorkspaceAssociative.status.label('status'))\
